...
const Lender = () => {
  ...
  const getAllBranchs = async () => {
    try {
      setLoading(true);
      const response = await getBanksByUserId(role, user_id);
      if (response.status === 200) {
        setBranchData(response.data);
      }
    } catch (error) {
  ...
  return (
    <Layout setSidebarCollapsed={setCollapsed}>
      <Suspense fallback={<div className="text-center mt-6">Loading header...</div>}>
        <PageHeader
          collapsed={collapsed}
          title="Lender"
          subTitle="(Branch/nbfc)"
      ...
      <div className="flex-1 flex flex-col items-center">
        {loading ? (
          ...
          <div className="container-lg m-4 mt-4 w-full max-w-7xl mx-auto grid grid-rows-1">
            {Array.isArray(BranchData) && BranchData.length > 0 ? (
              BranchData.map((Branch, index) => (
                <Suspense key={index} fallback={<div>Loading branch card...</div>}>
                  <BranchCard
                    data={Branch}
                    settingBranch={settingBranch}
                    setSettingBranch={setSettingBranch}
              ...
              <h1 className="text-center text-2xl font-bold mt-10">No Branch Found</h1>
            )}
          </div>
        )}
      </div>
